...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      dists[i,j] = torch.square(x_train[i] - x_test[j]).sum()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = torch.sum(x_train**2, dim=1).reshape(-1,1) +\
          torch.sum(x_test**2, dim=1).reshape(1,-1) - 2*torch.mm(x_train, x_test.t())
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      kx_test = x_train_folds[i]
      ky_test = y_train_folds[i]
      kx_train = torch.cat([x_train_folds[j] for j in range(num_folds)\
                if i!=j])
      ky_train = torch.cat([y_train_folds[j] for j in range(num_folds)\
                if i!=j])

      classifier = KnnClassifier(kx_train, ky_train)
      acc = classifier.check_accuracy(kx_test, ky_test, k, quiet=True)
